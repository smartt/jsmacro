...
__version__ = "0.2"
__doc__ = """
jsmacro is pre-processor desined for use with JavaScript.  (Where "macro" currently leans more toward the C definition of a macro than the Lisp definition.)
...
jsmacro doesn't bother to clean up extra whitespace or linebreaks that result in macro parsing, since that's the job of a JavaScript minifier (which in my case, is the tool that runs next in my build process, right after jsmacro.)
